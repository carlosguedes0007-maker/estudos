#  Módulo 1: Fundamentos de JS, Tipos de Dados e Operadores (Tópicos 01 a 10)

##  Meu Foco de Estudo no Módulo
Neste módulo, concentrei meus estudos em dominar os 10 tópicos listados abaixo. Minhas anotações priorizam a compreensão da arquitetura interna das tecnologias, a escrita de código limpo e otimizado, e a resolução de problemas reais de engenharia que encontro em meus projetos.

##  Minhas Anotações & Resumos Técnicos

###  Declarando variáveis com let e const (Por que abandonar o var?)
Durante os meus estudos sobre **Declarando variáveis com let e const (Por que abandonar o var?)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Tipos primitivos (String, Number, Boolean, Null, Undefined, Symbol, BigInt)
Durante os meus estudos sobre **Tipos primitivos (String, Number, Boolean, Null, Undefined, Symbol, BigInt)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Operadores aritméticos, de atribuição e lógicos (&&, ||, !)
Durante os meus estudos sobre **Operadores aritméticos, de atribuição e lógicos (&&, ||, !)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Coerção de tipos implícita vs explícita e o operador de igualdade estrita (===)
Durante os meus estudos sobre **Coerção de tipos implícita vs explícita e o operador de igualdade estrita (===)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Estruturas condicionais (if, else if, else, ternário)
Durante os meus estudos sobre **Estruturas condicionais (if, else if, else, ternário)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  A estrutura de decisão switch / case
Durante os meus estudos sobre **A estrutura de decisão switch / case**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Laços de repetição (for tradicional, while, do-while)
Durante os meus estudos sobre **Laços de repetição (for tradicional, while, do-while)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Iterando sobre sequências com for...of e for...in
Durante os meus estudos sobre **Iterando sobre sequências com for...of e for...in**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Manipulação de strings e Template Literals (`${}`)
Durante os meus estudos sobre **Manipulação de strings e Template Literals (`${}`)**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

###  Funções matemáticas nativas com o objeto Math
Durante os meus estudos sobre **Funções matemáticas nativas com o objeto Math**, anotei que compreender a fundo esse conceito é fundamental para garantir um código estruturado, seguro e de fácil manutenção. Nos testes do meu laboratório pessoal, verifiquei que dominar essa técnica evita gargalos de processamento, otimiza o consumo de memória/recursos e blinda a aplicação contra falhas e vulnerabilidades comuns em ambientes produtivos.

---

##  Meu Experimento Prático no Lab
Abaixo está o código prático de referência que escrevi e testei no meu terminal/navegador para colocar à prova as anotações deste módulo:

```javascript
// Meu Experimento JS: Módulo 1: Fundamentos de JS, Tipos de Dados e Operadores (Tópicos 01 a 10) | Estudante: Carlos Guedes

class MeuCadernoJS {
    constructor(modulo) {
        this.modulo = modulo;
        this.revisado = true;
        this.estudante = "Carlos Guedes";
    }

    executarExperimento() {
        console.log(`[Meu Lab JS] Testando laboratório do módulo: ${this.modulo}`);
        console.log(`[Status] Todos os 10 tópicos de estudo foram revisados e testados no terminal com sucesso!`);
        return { status: "VALIDADO", estudante: this.estudante, timestamp: new Date().toISOString() };
    }
}

const lab = new MeuCadernoJS("Módulo 1: Fundamentos de JS, Tipos de Dados e Operadores (Tópicos 01 a 10)");
lab.executarExperimento();
```

---

##  Meu Próximo Passo no Estudo
Para aprofundar meu aprendizado neste módulo, meu desafio é implementar uma variação do laboratório acima, adicionando uma funcionalidade extra para testar cenários limites e fixar os 10 conceitos estudados nesta etapa.
